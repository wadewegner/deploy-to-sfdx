    ...
    switch (command) {
      ...
      case 'create':
        ...
        commands.run(command, script, () => {
          ...
          script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:org:create -v '${access_token}' -s -f ${param} | jq -r .result`;
          ...
          commands.run(command, script, (commandResult, commandErr) => {
            ...
            if (commandErr) {
              ...
              res.json({
                message: `Org ${commandResult.orgId} created with User ${commandResult.username}`
              });
            }
      ...
      case 'apex':
        let message = 'Apex Scripts run:';
        for (let apex of param){
          console.log(`running ${apex}`);
          ...
          commands.run(command, script, (commandResult, commandErr) => {
            ...
            if (commandErr) {
              ...
              console.log(commandResult);
              // doesn't seem to be returning until after everything else runs.  fix UI output
              message = `${message}
              \t${apex}`;
            }
          });
      ...
      case 'permset':
        ...
        case 'password':
          ...
          script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:user:password:generate --json | jq -r .result.password`;
          ...
          commands.run(command, script, (commandResult, commandErr) => {
            ...
            if (commandErr) {
              ...
              res.json({
                message: `Your password for user ${user_name} is ${commandResult}.`
              });
            }
